...
tbm_begin_iterate(TIDBitmap *tbm)
{
	...
	struct 		timeval  tvBegin, tvEnd, tvDiff; /*renata: measure time interval for a step in the process*/
	...
	if (tbm->status == TBM_HASH && !tbm->iterating)
	{
		...
		/* renata */
		if (npages > 1){
			/* if smooth scan - check whether sorting is enabled
			 * if another operator - allow it */
			if ((enable_smoothscan && enable_smoothsorting) || !enable_smoothscan) {
				gettimeofday(&tvBegin, NULL);
				qsort(tbm->spages, npages, sizeof(PagetableEntry *),
					  tbm_comparator);
				gettimeofday(&tvEnd, NULL);
				SDBTimevalSubtract(&tvDiff, &tvEnd, &tvBegin);
				printf("\n Sort time for tidbitmap %ld pages: %ld.%06ld", npages, tvDiff.tv_sec, tvDiff.tv_usec);
			}
		}
		if (nchunks > 1){
			/* if smooth scan - check whether sorting is enabled
			...
			if ((enable_smoothscan && enable_smoothsorting) || !enable_smoothscan) {
				...
				gettimeofday(&tvEnd, NULL);
				SDBTimevalSubtract(&tvDiff, &tvEnd, &tvBegin);
				printf("\n Sort time for tidbitmap %ld chunks: %ld.%06ld", nchunks, tvDiff.tv_sec, tvDiff.tv_usec);
			}
		}
	}
