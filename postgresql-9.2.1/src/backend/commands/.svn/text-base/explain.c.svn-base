			...
			ExplainState *es)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_IndexSmoothScan:
			pname = sname = "Index Smooth Scan";
			break;
		case T_SmoothIndexScan:
			pname = sname = "Smooth Index Scan";
			break;
		case T_SmoothHeapScan:
			pname = sname = "Smooth Heap Scan";
			break;
	...
	switch (nodeTag(plan))
	{
		...
		case T_IndexSmoothScan:
			{
				IndexSmoothScan  *indexscan = (IndexSmoothScan *) plan;
		...
		case T_SmoothIndexScan:
			{
				SmoothIndexScan *smoothindexscan = (SmoothIndexScan *) plan;
				const char *indexname =
				explain_get_index_name(smoothindexscan->sis_indexid);
	...
	switch (nodeTag(plan))
	{
		case T_IndexScan:
			...
			break;
		case T_IndexSmoothScan:
			show_scan_qual(((IndexSmoothScan *) plan)->indexqualorig,
						   "Index Cond", planstate, ancestors, es);
			if (((IndexSmoothScan *) plan)->indexqualorig)
				show_instrumentation_count("Rows Removed by Index Recheck", 2,
										   planstate, es);
			show_scan_qual(((IndexSmoothScan *) plan)->indexorderbyorig,
						   "Order By", planstate, ancestors, es);
			show_scan_qual(plan->qual, "Filter", planstate, ancestors, es);
			if (plan->qual)
				show_instrumentation_count("Rows Removed by Filter", 1,
		...
		case T_SmoothIndexScan:
			show_scan_qual(((SmoothIndexScan *) plan)->sis_indexqualorig,
						   "Index Cond", planstate, ancestors, es);
			break;
		case T_SmoothHeapScan:
			show_scan_qual(((SmoothHeapScan *) plan)->shs_bitmapqualorig,
						   "Recheck Cond", planstate, ancestors, es);
			if (((SmoothHeapScan *) plan)->shs_bitmapqualorig)
				show_instrumentation_count("Rows Removed by Index Recheck", 2,
										   planstate, es);
			show_scan_qual(plan->qual, "Filter", planstate, ancestors, es);
			if (plan->qual)
