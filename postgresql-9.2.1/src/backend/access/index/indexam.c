...
//							// Start smooth from beginning
						...
						if (valid){
							...
							if(smooth_resultcache_add_tuple(smoothDesc->result_cache, page, lineoff, tuple, RelationGetDescr(scan->heapRelation), target_list, qual_list, index)){
								smoothDesc->prefetch_counter++;
								smoothDesc->smooth_counter++;
								//17.02.2014
								//increase the counter just for the first time we calculate this page
				...
				if(num_tuples_prefetch >= 0){
					...
					if(!enable_skewcheck || !used_prefetcher){
						...
						if (enable_benchmarking)
							printf("\n Normal Smooth Scan. Number of pages checked %ld. Prefetch target %ld  ", bms_num_members(smoothDesc->bs_vispages), smoothDesc->prefetch_target);
