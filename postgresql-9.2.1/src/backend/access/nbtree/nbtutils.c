...
_bt_preprocess_array_keys(IndexScanDesc scan)
{
	...
	if(ss != NULL){
		...
		if (ss->arrayContext == NULL)
				ss->arrayContext = AllocSetContextCreate(CurrentMemoryContext,
														 "BTree Array Smooth Context",
														 ALLOCSET_SMALL_MINSIZE,
														 ALLOCSET_SMALL_INITSIZE,
														 ALLOCSET_SMALL_MAXSIZE);
			else
				MemoryContextReset(ss->arrayContext);
	}
	...
	if(ss != NULL){
		/************************************************************************************/
		//now do the same for Smooth Scan
		MemoryContextSwitchTo(ss->arrayContext);
		/* Create modifiable copy of scan->keyData in the workspace context */
		ss->arrayKeyData = (ScanKey) palloc(scan->numberOfKeys * sizeof(ScanKeyData));
		memcpy(ss->arrayKeyData,
			   scan->keyData,
			   scan->numberOfKeys * sizeof(ScanKeyData));

		/* Allocate space for per-array data in the workspace context */
		ss->arrayKeys = (BTArrayKeyInfo *) palloc0(numArrayKeys * sizeof(BTArrayKeyInfo));


		for (i = 0; i < so->numArrayKeys; i++)
		{
			ss->arrayKeys[i].scan_key = so->arrayKeys[i].scan_key;
...
_bt_start_array_keys(IndexScanDesc scan, ScanDirection dir)
{
	...
	if(ss != NULL){
		for (i = 0; i < ss->numArrayKeys; i++)
			{
				BTArrayKeyInfo *curArrayKey = &ss->arrayKeys[i];
				ScanKey		skey = &ss->arrayKeyData[curArrayKey->scan_key];
		...
		for (i = ss->numArrayKeys - 1; i >= 0; i--)
			{
				...
				if (ScanDirectionIsBackward(dir))
				{
					...
					{
						cur_elem = num_elems - 1;
						foundSmooth = false;	/* need to advance next array key */
					}
					else
						foundSmooth = true;
				}
...
_bt_preprocess_keys(IndexScanDesc scan)
{
	...
	if (numberOfKeys == 1)
	{
		...
		if(ss != NULL){
			ss->numberOfKeys = 1;
			memcpy(&ss->keyData[0], cur, sizeof(ScanKeyData));
		}
		return;
	}
	...
	//renata: do the same for Smooth Keys
	// todo - this copies values directly!!
	if (ss != NULL){
		ss->numberOfKeys = so->numberOfKeys;
		for ( i = 0; i < so->numberOfKeys ; i++ )
		{	ScanKey		skey = &(ss->keyData[i]);
			memcpy(skey, &so->keyData[i], sizeof(ScanKeyData));
		}
	}
}
