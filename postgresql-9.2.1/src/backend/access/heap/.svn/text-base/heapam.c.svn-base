...
bool HeapKeyTestSmooth(HeapTuple tuple, TupleDesc tupdesc, int nkeys, ScanKey keys)
{
	bool result;
	...
	do
	{
		...
		for (; __cur_nkeys--; __cur_keys++)
		{
			...
			if (enable_smoothnestedloop && __cur_keys->sk_belongstojoin) /* ignore this check if it is a part of join */
				{
					continue;
				}
...
simple_heap_update(Relation relation, ItemPointer otid, HeapTuple tup)
{
	...
	switch (result)
	{
		case HeapTupleSelfUpdated:
			/* Tuple was already updated in current command? */
			/* renata : allow this just to see what will happen - TODO remove later*/
			//elog(ERROR, "tuple already updated by self");
			break;
