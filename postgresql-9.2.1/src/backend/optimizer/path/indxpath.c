...
static void get_index_paths(PlannerInfo *root, RelOptInfo *rel,
				...
				List **bitindexpaths, ScanTypeControl scantype);
/* renata */
static void get_indexsmooth_paths(PlannerInfo *root, RelOptInfo *rel,
				IndexOptInfo *index, IndexClauseSet *clauses,
				List **bitindexpaths, ScanTypeControl scantype);
static void
			get_smooth_index_paths(PlannerInfo *root, RelOptInfo *rel,
				IndexOptInfo *index, IndexClauseSet *clauses,
				List **bitindexpaths);
...
static List *build_indexsmooth_paths(PlannerInfo *root, RelOptInfo *rel,
				  IndexOptInfo *index, IndexClauseSet *clauses,
				  bool useful_predicate,
				  SaOpControl saop_control, ScanTypeControl scantype);
...
void
create_indexsmooth_paths(PlannerInfo *root, RelOptInfo *rel)
{
	List	   *indexpaths;
	List	   *bitindexpaths;
	List	   *bitjoinpaths;
	...
	foreach(ilist, rel->indexlist)
	{
		...
		//get_index_paths(root, rel, index, &rclauseset,
		//				&bitindexpaths, ST_ANYSCAN);
		get_indexsmooth_paths(root, rel, index, &rclauseset, &bitindexpaths, ST_INDEXSCAN);
	...
	/**********************************************************************************/
	/*  for Q1 -IT WORKS WHEN THIS IS COMMENTED
//		 * Generate BitmapOrPaths for any suitable OR-clauses present in the
//		 * restriction list.  Add these to bitindexpaths.
//		 */
//		indexpaths = generate_bitmap_or_paths(root, rel,
...
/* renata
 * create_smoothscan_paths()
 *	  Generate all interesting smoothscan paths for the given relation.
 *	  Candidate paths are added to the rel's pathlist (using add_path).
 *
 * To be considered for a smooth scan, an index must match one or more
 * restriction clauses or join clauses from the query's qual condition,
 * or match the query's ORDER BY condition, or have a predicate that
 * matches the query's qual condition.
 *
 ...
 * attributes are available and fixed during any one scan of the indexpath.
 *
 * A SmoothScanPath is generated and submitted to add_path() for each plain or
 * parameterized index scan this routine deems potentially interesting for
 * the current query.
 *
 * 'rel' is the relation for which we want to generate smooth scan paths
 *
 * Note: check_partial_indexes() must have been run previously for this rel.

 */
void
create_smooth_scan_paths(PlannerInfo *root, RelOptInfo *rel)
{
	List	   *indexpaths;
	List	   *bitindexpaths;
	List	   *bitjoinpaths;
	...
	foreach(ilist, rel->indexlist)
	{
		 ...
		 * bitmap paths are added to bitindexpaths to be handled below.
		 */
		get_smooth_index_paths(root, rel, index, &rclauseset,
								&bitindexpaths);
	 ...
	 */
	if (bitindexpaths != NIL)
	{	//todo: maybe for now please only plain  BitmapHeapPath
		Path	   *bitmapqual;
		SmoothHeapPath *spath;

		bitmapqual = choose_bitmap_and(root, rel, bitindexpaths);
		spath = create_smooth_heap_path(root, rel, bitmapqual, NULL, 1.0);
	...
	if (bitjoinpaths != NIL)
	{
		...
		foreach(lc, all_path_outers)
		{
			...
			required_outer = get_bitmap_tree_required_outer(bitmapqual);
			loop_count = get_loop_count(root, required_outer);
			spath = create_smooth_heap_path(root, rel, bitmapqual,
											required_outer, loop_count);
			add_path(rel, (Path *) spath);
		}
	}
					 ...
					 List **considered_relids)
{
	...
	/*************************************************************************/
	//todo check this 13.02.2014
	//if(!enable_indexsmoothscan)
		get_index_paths(root, rel, index, &clauseset, bitindexpaths, ST_ANYSCAN);
	//else
	//	get_indexsmooth_paths(root, rel, index, &clauseset, bitindexpaths, ST_INDEXSCAN);
	/**************************************************************************/
	/*
				...
				List **bitindexpaths)
{
	...
	indexpaths = build_index_paths(root, rel,
								   index, clauses,
								   index->predOK,
								   SAOP_PER_AM,  ST_BITMAPSCAN);
	/*
	 *
	 * Pick out indices that are usable as bitmap scans.  For that, we
...
static void
get_indexsmooth_paths(PlannerInfo *root, RelOptInfo *rel,
				IndexOptInfo *index, IndexClauseSet *clauses,
				List **bitindexpaths, ScanTypeControl scantype)
{
	List	   *indexpaths;
	 ...
	 * clauses only if the index AM supports them natively.
	 */
//	indexpaths = build_indexsmooth_paths(root, rel,
//								   index, clauses,
//								   index->predOK,
//								   SAOP_PER_AM);
	/********************************************************************/
	...
	foreach(lc, indexpaths)
	{
		IndexSmoothPath  *ipath = (IndexSmoothPath *) lfirst(lc);

		if (index->amhasgettuple){
			add_path(rel, (Path *) ipath);
			printf("indexsmoothpath is added with add path. from get_indexsmooth_paths\n");
		}
...
static List *
build_indexsmooth_paths(PlannerInfo *root, RelOptInfo *rel,
				  IndexOptInfo *index, IndexClauseSet *clauses,
				  bool useful_predicate,
				  SaOpControl saop_control, ScanTypeControl scantype)
{
	List	   		*result = NIL;
	IndexSmoothPath	*ipath;
	IndexPath		*ionlyPath;
	List	  		*index_clauses;
	List	   		*clause_columns;
	Relids			outer_relids;
	 ...
	 * many of them are actually useful for this query.
	...
	{
			if(index_only_scan){
				ionlyPath = create_index_path(root, index,
											  index_clauses,
											  clause_columns,
											  orderbyclauses,
											  orderbyclausecols,
				...
				result = lappend(result, ionlyPath);
			}else{
				printf("find relevant restriction clause for index. calling create_index_smooth_path\n");
				ipath = create_index_smooth_path(root, index,
										  index_clauses,
										  clause_columns,
										  orderbyclauses,
										  orderbyclausecols,
	...
	if (index_is_ordered && pathkeys_possibly_useful)
	{
		...
		{
				if(index_only_scan){
					/* index only scan should be a regular index not smooth*/
					ionlyPath = create_index_path(root, index,
												  index_clauses,
												  clause_columns,
												  NIL,
...
/* renata
 * todo
 * build cost similar to bitmap heap scan (for the case when child is index access )*/
/*
 * Estimate the cost of actually executing a smooth heap scan with a single
 * index path (no BitmapAnd, at least not at this level; but it could be
 * a BitmapOr).
 */
static Cost
smooth_scan_cost_est(PlannerInfo *root, RelOptInfo *rel, Path *ipath)
{
	SmoothHeapPath 	spath;
	Relids			required_outer;
	...
	/* Set up a dummy SmoothHeapPath */
	spath.path.type = T_SmoothHeapPath;
	spath.path.pathtype = T_SmoothHeapScan;
	spath.path.parent = rel;
	spath.path.param_info = get_baserel_parampathinfo(root, rel,
													  required_outer);
	spath.path.pathkeys = NIL;
	spath.shp_bitmapqual = ipath;

	cost_smooth_heap_scan(&spath.path, root, rel,
						  spath.path.param_info,
						  ipath,
						  get_loop_count(root, required_outer));

	return spath.path.total_cost;
}
...
get_bitmap_tree_required_outer(Path *bitmapqual)
{
	...
	/************************************************************/
	else if (IsA(bitmapqual, IndexSmoothPath))
	{
		return bms_copy(PATH_REQ_OUTER(bitmapqual));
	}
	/************************************************************/
...
find_indexpath_quals(Path *bitmapqual, List **quals, List **preds)
{
	...
	{
		IndexSmoothPath  *ipath = (IndexSmoothPath *) bitmapqual;
