...
bool		enable_smoothsorting 	= false;
...
// x: at this point!!
int			num_tuples_prefetch = -1;
//THESE SHOULD BE LOCAL VARIABLES PER INDEX 13.02.2014
...
cost_index(IndexPath *path, PlannerInfo *root, double loop_count)
{
	...
	/* this is called from smooth scan as well */
	if (!enable_indexscan && !indexonly )
		startup_cost += disable_cost;
...
void
cost_index_smooth(IndexSmoothPath *path, PlannerInfo *root, double loop_count)
{
	IndexOptInfo *index = path->indexinfo;
	RelOptInfo *baserel = index->rel;
	...
	path->path.startup_cost = startup_cost;
	path->path.total_cost = startup_cost + run_cost;
	printf("\nIndex smooth scan total cost is  %lf\n",path->path.total_cost  );
}

/* renata
 * cost_smoothscan
 *	  Determines and returns the cost of scanning a relation using a smooth scan.
 *
 * 'path' describes the indexscan under consideration, and is complete
 *		except for the fields to be set by this routine
 * 'loop_count' is the number of repetitions of the indexscan to factor into
 ...
 * we have to fetch from the table, so they don't reduce the scan cost.
 *
 * NOTE2: for now this is a copy of cost_index
 * todo: later change this cost a bit, it should be a bit increased in comparison with index !!!
 * todo: here comes the cost model for smooth scan !!!
 */
//void
//cost_smoothscan(SmoothScanPath *path, PlannerInfo *root, double loop_count)
//{
//	IndexOptInfo *index = path->indexinfo;
//	RelOptInfo *baserel = index->rel;
//
...
//	}
//
//	if (!enable_smoothscan)
//		startup_cost += disable_cost;
//
//	/* renata todo: for smooth scan those methods should be different */
//	/*
//	 * Call index-access-method-specific code to estimate the processing cost
//	 * for scanning the index, as well as the selectivity of the index (ie,
//	 * the fraction of main-table tuples we will have to retrieve) and its
...
//		 */
//
//		/* renata todo: in smooth scan pages differ - for now this is only for cost model,
...
/*
 * cost_smooth_heap_scan
 *	  Determines and returns the cost of scanning a relation using a smooth
 *	  index-then-heap plan.
 *
 * 'baserel' is the relation to be scanned
 * 'param_info' is the ParamPathInfo if this is a parameterized path, else NULL
 ...
 */
void
cost_smooth_heap_scan(Path *path, PlannerInfo *root, RelOptInfo *baserel,
					  ParamPathInfo *param_info,
					  Path *bitmapqual, double loop_count)
{
	Cost		startup_cost = 0;
...
cost_bitmap_tree_node(Path *path, Cost *cost, Selectivity *selec)
{
	...
	//renata
	/**************************************************/
	else if (IsA(path, IndexSmoothPath))
	{
		*cost = ((IndexSmoothPath *) path)->indextotalcost;
		*selec = ((IndexSmoothPath *) path)->indexselectivity;
...
has_indexed_join_quals(NestPath *joinpath)
{
	...
	switch (innerpath->pathtype)
	{
		...
		case T_IndexSmoothScan:
			indexclauses = ((IndexSmoothPath *) innerpath)->indexclauses;
			break;

		case T_SmoothHeapScan:
			{
				/* Accept only a simple bitmap scan, not AND/OR cases */
				Path	   *smqual = ((SmoothHeapPath *) innerpath)->shp_bitmapqual;

				if (IsA(smqual, IndexPath))
					indexclauses = ((IndexPath *) smqual)->indexclauses;
				else
					return false;
				break;
			}
