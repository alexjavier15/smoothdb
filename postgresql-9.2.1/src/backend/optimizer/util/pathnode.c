...
//		if (path->param_info)
//		{	printf("path with cost %lf is parametrized path\n",path->total_cost );
//
...
//			printf("cheapest total is null. set to first which cost is %lf\n", path->total_cost);
//			continue;
//		}
//		/*todo renata remove this. for now only for testing purpose */
//		printf("\nPath considered with cost %lf\n",path->total_cost);
//
//		/*
...
//		printf("compare cheapest total (%lf) with current path (%lf). Result is %d (>0 means second is better)", cheapest_total_path->total_cost, path->total_cost, cmp);
//		if (cmp > 0 ||
//			(cmp == 0 &&
...
//	 * todo remove later*/
//	//elog_node_display(INFO, "Cheapest path found", cheapest_total_path,
//	//										  true);
//
...
/*
 * create_index_smooth_path
 *	  Creates a path node for an index smooth scan.
 *
 * 'index' is a usable index.
 * 'indexclauses' is a list of RestrictInfo nodes representing clauses
 *			to be used as index qual conditions in the scan.
 ...
 */
IndexSmoothPath *
create_index_smooth_path(PlannerInfo *root,
				  IndexOptInfo *index,
				  List *indexclauses,
				  List *indexclausecols,
				  List *indexorderbys,
				  ...
				  double loop_count)
{
	/* todo: see maybe for index only path to generate IndexPath instead*/
	IndexSmoothPath  *pathnode = makeNode(IndexSmoothPath);
	RelOptInfo *rel = index->rel;
	List	   *indexquals,
			   *indexqualcols;

	printf("creating index smooth scan path node \n");

	pathnode->path.pathtype = indexonly ? T_IndexOnlyScan : T_IndexSmoothScan;
	pathnode->path.parent = rel;
	pathnode->path.param_info = get_baserel_parampathinfo(root, rel,
														  required_outer);
	pathnode->path.pathkeys = pathkeys;
	...
	cost_index_smooth(pathnode, root, loop_count);
...
/*
 * create_smooth_heap_path
 *	  Creates a path node for a smooth heap scan.
 *
 ...
 */
SmoothHeapPath *
create_smooth_heap_path(PlannerInfo *root,
						RelOptInfo *rel,
						Path *bitmapqual,
						Relids required_outer,
						double loop_count)
{
	SmoothHeapPath *pathnode = makeNode(SmoothHeapPath);

	pathnode->path.pathtype = T_SmoothHeapScan;
	pathnode->path.parent = rel;
	pathnode->path.param_info = get_baserel_parampathinfo(root, rel,
														  required_outer);
	pathnode->path.pathkeys = NIL;		/* always unordered */

	pathnode->shp_bitmapqual = bitmapqual;

	cost_smooth_heap_scan(&pathnode->path, root, rel,
...
calc_non_nestloop_required_outer(Path *outer_path, Path *inner_path)
{
	...
	/********************************************************************/
	//renata
//	//todo: i think || or should go here, but then I got an error that a plan cannot be devised
//	if(!bms_overlap(outer_paramrels, inner_path->parent->relids) && !bms_overlap(inner_paramrels, outer_path->parent->relids)){
//		return NULL;
//	}
	/********************************************************************/
	/* form the union ... */
					...
					double loop_count)
{
	...
	switch (path->pathtype)
	{
		...
		case T_IndexSmoothScan:
			{
				IndexSmoothPath  *ipath = (IndexSmoothPath *) path;
				IndexSmoothPath  *newpath = makeNode(IndexSmoothPath);
				 ...
				 * the cost estimate.
				 */
				memcpy(newpath, ipath, sizeof(IndexSmoothPath));
				newpath->path.param_info =
					get_baserel_parampathinfo(root, rel, required_outer);
				cost_index_smooth(newpath, root, loop_count);
				return (Path *) newpath;
			}
		case T_SmoothHeapScan:
			{
				SmoothHeapPath *spath = (SmoothHeapPath *) path;

				return (Path *) create_smooth_heap_path(root,
														rel,
														spath->shp_bitmapqual,
														required_outer,
														loop_count);
			}
