...
set_plan_refs(PlannerInfo *root, Plan *plan, int rtoffset)
{
	...
	switch (nodeTag(plan))
	{
		...
		case T_IndexSmoothScan:
			{
				IndexSmoothScan  *splan = (IndexSmoothScan *) plan;
		...
		case T_SmoothIndexScan:
			{
				SmoothIndexScan *splan = (SmoothIndexScan *) plan;
				...
				splan->sis_indexqual =
					fix_scan_list(root, splan->sis_indexqual, rtoffset);
				splan->sis_indexqualorig =
					fix_scan_list(root, splan->sis_indexqualorig, rtoffset);
			}
			break;
		case T_SmoothHeapScan:
			{
				SmoothHeapScan *splan = (SmoothHeapScan *) plan;
				...
				splan->scan.plan.qual =
					fix_scan_list(root, splan->scan.plan.qual, rtoffset);
				splan->shs_bitmapqualorig =
					fix_scan_list(root, splan->shs_bitmapqualorig, rtoffset);
			}
			break;
