...
ExecInitNode(Plan *node, EState *estate, int eflags)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_IndexScan:
			//renata 13.02.2014 - check this todo
			if(!enable_indexsmoothscan){
				result = (PlanState *) ExecInitIndexScan((IndexScan *) node,
													 estate, eflags);
			}else{
				result = (PlanState *) ExecInitIndexSmoothScan((IndexSmoothScan *) node,
																	 estate, eflags);
			}
			break;
		...
		case T_SmoothIndexScan:
			result = (PlanState *) ExecInitSmoothIndexScan((SmoothIndexScan *) node,
														   estate, eflags);
			break;

		case T_SmoothHeapScan:
			result = (PlanState *) ExecInitSmoothHeapScan((SmoothHeapScan *) node,
														  estate, eflags);
			break;
...
ExecProcNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_IndexSmoothScanState:
			result = ExecIndexSmoothScan((IndexSmoothScanState *) node);
			break;
		case T_SmoothHeapScanState:
			result = ExecSmoothHeapScan((SmoothHeapScanState *) node);
			break;
...
MultiExecProcNode(PlanState *node)
{
	...
	switch (nodeTag(node))
	{
		...
		case T_SmoothIndexScanState:
			result = MultiExecSmoothIndexScan((SmoothIndexScanState *) node);
			break;
