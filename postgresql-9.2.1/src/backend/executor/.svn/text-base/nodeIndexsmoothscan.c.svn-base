...
HeapTuple project_tuple(const HeapTuple tuple, const TupleDesc tupleDesc, List *target_list, List *qual_list, Index index, Datum *values, bool * isnull);
//
///*Smooth Operators
...
{	HASHCTL			hash_ctl;
	...
	tupleCache->hashtable = hash_create("TIDCache",
								 128,	/* start small and extend */
								 &hash_ctl,
...
//		 + sizeof(Pointer) + sizeof(Pointer));
	...
	//this one works
	nbuckets = res_cache->size /
			(MAXALIGN(sizeof(HASHELEMENT)) + MAXALIGN(sizeof(ResultCacheKey)+ (tup_length))
			 + sizeof(Pointer) + sizeof(Pointer));
...
//				 + sizeof(Pointer) + sizeof(Pointer));
	...
	/* Create the hashtable proper */
	MemSet(&hash_ctl, 0, sizeof(hash_ctl));
	hash_ctl.keysize = sizeof(ResultCacheKey);
...
bool smooth_tuplecache_add_tuple(TupleIDCache * cache, const TID tupleID)
{
	...
	resultEntry = (TupleIDCacheEntry *) hash_search(cache->hashtable,
													  (void *) &tupleID,
													  HASH_ENTER, &found);
	if(resultEntry != NULL){
...
smooth_resultcache_add_tuple(ResultCache *cache, const BlockNumber blknum, const OffsetNumber off, const HeapTuple tpl, const TupleDesc tupleDesc, List *target_list, List *qual_list, Index index)
{
		...
		HeapTuple projectedTuple = project_tuple(tpl, tupleDesc, target_list, qual_list, index, cache->projected_values, cache->projected_isnull);

		resultEntry = smooth_resultcache_get_resultentry(cache, projectedTuple, blknum);
...
//		if (!is_target_attribute(thisatt, target_list))
	// 2014 - todo sigmod 2015 - new with bitmap
		if(!found){
			if(qual_list == NULL || (!bms_is_member((thisatt->attnum - FirstLowInvalidHeapAttributeNumber), attrs_used)))
			{	//FOR THE ONES I DON'T NEED - JUST SET IS NULL TO YES - AND HOPEFULLY I WON'T PICK IT UP WHEN I FORM NEW TUPLE
...
IndexSmoothNext(IndexSmoothScanState *node)
{
	...
	while ((tuple = indexsmooth_getnext(scandesc, direction,  node->ss.ps.plan->plan_rows, node->iss_NumScanKeys, node->iss_ScanKeys, node->ss.ps.targetlist,  node->ss.ps.qual,((IndexSmoothScan *)(node->ss.ps.plan))->scan.scanrelid,  node->iss_NumSmoothScanKeys, node->iss_SmoothScanKeys, node->allqual, node->ss.ps.ps_ExprContext,  node->ss.ss_ScanTupleSlot)) != NULL)
	{
		/*
...
{	SmoothScanOpaque ss;
	...
	if (indexScanDesc != NULL){
		...
		//	ss->bs_tovispages = NULL;
			...
			//RECREATE
			if (ss->orderby){
				ss->result_cache = smooth_resultcache_create_empty(work_mem * 1024L, RelationGetDescr(indexScanDesc->heapRelation)->natts);
			}else{
	...
	if (node->iss_NumRuntimeSmoothKeys != 0)
	{
		...
		ResetExprContext(econtext);
		ExecIndexEvalRuntimeKeys(econtext,
								 node->iss_RuntimeSmoothKeys,
								 node->iss_NumRuntimeSmoothKeys);
	}
...
ExecEndIndexSmoothScan(IndexSmoothScanState *node)
{
	...
	if (indexScanDesc != NULL){
		...
		if(ss->tupleID_cache !=NULL)
			printf("\n Table ID cache size %ld, number of entries %ld", hash_estimate_size(ss->tupleID_cache->nentries, sizeof(TupleIDCacheEntry) ), ss->tupleID_cache->nentries);

		printf("\n Number of result cache (tableID) hits: %ld, out of total number of tuples: %ld \n", ss->num_result_cache_hits, ss->num_result_tuples);
		/*delete smooth info*/
...
ExecInitIndexSmoothScan(IndexSmoothScan *node, EState *estate, int eflags)
{
	...
	indexstate->allqual= (List *)
			ExecInitExpr((Expr *) node->allqual,
						 (PlanState *) indexstate); //smooth scan needs to keep all predicates
...
///******************************************************/
	...
	ExecIndexBuildScanKeys((PlanState *) indexstate,
							   ...
							   node->indexorderby,
							   true,
							   &indexstate->iss_OrderBySmoothKeys,
							   &indexstate->iss_NumOrderBySmoothKeys,
							   &indexstate->iss_RuntimeSmoothKeys,
							   &indexstate->iss_NumRuntimeSmoothKeys,
							   NULL,	/* no ArrayKeys */
							   NULL);
