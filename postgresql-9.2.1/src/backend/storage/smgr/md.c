...
/* renata
 * For Index Smooth Scan we had to adapt the function, to read a chunk of blocks, instead of just ONE
 * Usually the disk controler is supposed to notice sequential access, but since it didn't in our case
 * this is a work around
 *mdprefetch() -- Initiate asynchronous read of the specified block of a relation
 */

void
mdprefetch_smooth(SMgrRelation reln, ForkNumber forknum, BlockNumber blocknum, int num_blocks)
{
#ifdef USE_PREFETCH
	off_t		seekpos;
	MdfdVec    *v;
	...
	(void) FilePrefetch(v->mdfd_vfd, seekpos, (num_blocks * BLCKSZ));
#endif   /* USE_PREFETCH */
}
