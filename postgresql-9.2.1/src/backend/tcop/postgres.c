...
pg_plan_query(Query *querytree, int cursorOptions, ParamListInfo boundParams)
{
	...
	/* renata: print final plan stmt
	 *
	 * */
	elog_node_display(INFO, "Plan after calling the optimizer", plan,
								  true);
...
exec_simple_query(const char *query_string)
{
	...
	struct timeval  tvAllBegin, tvAllEnd, tvAllDiff; /* whole execute query statements*/
	...
	foreach(parsetree_item, parsetree_list)
	{
		...
		List	   *querytree_list, *backupquery_list,
				   *plantree_list;
		Portal		portal;
		DestReceiver *receiver;
		int16		format;
		int 		counter = 0; /* renata: TODO this counter should be removed once the actual logic is added */
...
//-------------------------------------------------------------------------
		/* renata: repeat steps - plan query, execute, plan query, execute until reoptimize = false;
		 * */
		do {
			counter++;

			/* renata: get plans always in message context*/
			if (analyze_requires_snapshot(parsetree))
					{
						PushActiveSnapshot(GetTransactionSnapshot());
						snapshot_set = true;
			...
			/* renata gather time information */
			...
			printf("\nGet new plan (analyze+plan): %ld.%06ld", tvDiff.tv_sec, tvDiff.tv_usec);
			//ereport(INFO,(errmsg_internal("Get new plan (analyze+plan): %ld.%06ld", tvDiff.tv_sec, tvDiff.tv_usec)));
//-----------------------------------------------------------------------
			/* renata : print final plan tree list
			 * */
			elog_node_display(INFO, "Plan tree list", plantree_list,
										  true);
...
//----------------------------------------------------
			switch (portal->strategy)
			{
				case PORTAL_ONE_SELECT:
					...
					reoptimize = portal->queryDesc->reoptimize;
					if(reoptimize && materialize)
						parsetree = (Node *)copyObject(portal->queryDesc->newParseTree);
					break;
...
//----------------------------------------------------
	...
	printf("\nWhole execute query command: %ld.%06ld\n", tvAllDiff.tv_sec, tvAllDiff.tv_usec);
	//printf("\nTime spent on IO: %ld\n",  (&planstate->instrument->bufusage->blk_read_time);
	//ereport(INFO,(errmsg_internal("Whole execute query command: %ld.%06ld\n", tvAllDiff.tv_sec, tvAllDiff.tv_usec )));
