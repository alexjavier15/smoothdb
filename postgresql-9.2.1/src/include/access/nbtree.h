...
/* RENATA - SMOOTH OPERATORS LOGIC STARTS HERE */
typedef enum
{
	SS_EMPTY,				/* no hashtable, nentries == 0 */
	SS_HASH,				/* we have has table */
	SS_FULL 				/* hash table is full - we cannot add more tuples to it
...
#define form_tuple_id(tpl, blknum, tid)\
	...
	*(tid) = (TID) ((__temp << 32) | ((uint32)((HeapTuple)(tpl))->t_self.ip_posid));

/* renata new */
//typedef uint32 TID_SHORT;
///* max offset number is 255 - therefore we will try to place both blk number and tuple id in 32 bit so that we could use it for bitmap structure */
//#define form_tuple_id_short(tpl, blknum, tid)\
//	TID_SHORT __temp = (TID_SHORT) (blknum); \
//	*(tid) = (TID_SHORT) ((__temp << 8) | ((uint8)((HeapTuple)(tpl))->t_self.ip_posid));


/* one element of a hash table (one bucket)*/
typedef struct ResultCacheTuple
{
	LocationIndex 	tupleOffset;		/* how far is this tuple tuple */
	ItemPointerData heapTID;			/* tuple ID*/
} ResultCacheTuple;

/* one element of a hash table (one bucket)*/
/* key = blok id, value = list of tuples from a page */
//typedef struct ResultCacheEntry
//{
//	BlockNumber 		blockID;		/* Block id number (hashtable key) */
//	char				tuples[BLCKSZ];	/* value - tuples from that page */
//	int 				numTuples;		/* how many tuples from that page qualify */
//	int    				nextTupleOffset; /* where next tuple should be placed */
//	ResultCacheTuple	tupleInfo[MaxHeapTuplesPerPage];	/* how far is this tuple tuple */
//} ResultCacheEntry;

/* one element of a hash table (one bucket)*/
/* key = TID, value  = tuple 1:1 */
typedef TID ResultCacheKey;
...
/* one element of a hash table (one bucket)*/
/* key = tid, value  = 0, 1 present or not  */
typedef struct TupleIDCacheEntry
{
	TID	 				tid;				/* Tuple id number (hashtable key) */
	uint8     			valid;				/* is tuple still present? */
} TupleIDCacheEntry;
...
struct TupleIDCache
{
	...
	HTAB	   *hashtable;		/* hash table of TableIDCacheEntry */

};

typedef struct TupleIDCache TupleIDCache;
