...
/* ----------------
 * renata
 *		index smooth scan node
 *
 * this node is a copy of a index scan - adapted to do index smooth scan
 * indexqualorig is an implicitly-ANDed list of index qual expressions, each
 * in the same form it appeared in the query WHERE condition.  Each should
 * be of the form (indexkey OP comparisonval) or (comparisonval OP indexkey).
 * The indexkey is a Var or expression referencing column(s) of the index's
 ...
 * ----------------
 */
typedef struct IndexSmoothScan
{
	Scan		scan;
	Oid			indexid;		/* OID of index to scan */
	List	   *indexqual;		/* list of index quals (usually OpExprs) */
	...
	List	   *indexorderby;	/* list of index ORDER BY exprs */
	List	   *indexorderbyorig;		/* the same in original form */
	List	   *allqual;     // Smooth: list of all quals
	ScanDirection indexorderdir;	/* forward or backward or don't care */
	bool        orderby;				/* should we maintain order? (if the statement has order by)*/
} IndexSmoothScan;

/* renata todo: change adapt*/
/* ----------------
 *		smooth index scan node
 *
 * SmoothIndexScan delivers a bitmap of potential tuple locations;
 * it does not access the heap itself.	The bitmap is used by an
 * ancestor SmoothHeapScan node,[ possibly after passing through
 * intermediate BitmapAnd and/or BitmapOr nodes to combine it with
 * the results of other BitmapIndexScans - does not hold for smoothscan ]
 *
 * The fields have the same meanings as for IndexScan, except we don't
 * store a direction flag because direction is uninteresting. - FOR NOW todo
 *
 * In a SmoothIndexScan plan node, the targetlist and qual fields are
 * not used and are always NIL.  The indexqualorig field is unused at
 * run time too, but is saved for the benefit of EXPLAIN.
 * ----------------
 */
typedef struct SmoothIndexScan
{
	Scan		scan;
	Oid			sis_indexid;		/* OID of index to scan */
	List	   *sis_indexqual;		/* list of index quals (OpExprs) */
	List	   *sis_indexqualorig;	/* the same in original form */
} SmoothIndexScan;

/* ----------------
 *		smooth sequential scan node
 *
 ...
 * ----------------
 */
typedef struct SmoothHeapScan
{
	Scan		scan;
	List	   *shs_bitmapqualorig; /* index quals, in standard expr form */
} SmoothHeapScan;
