...
/*----------
 * renata todo: change this for now the same as indexPath
 * IndexPath represents an index scan over a single index.
 *
 * This struct is used for both regular indexscans and index-only scans;
 * path.pathtype is T_IndexScan or T_IndexOnlyScan to show which is meant.
 ...
 *----------
 */
typedef struct IndexSmoothPath
{
	Path		path;
	IndexOptInfo *indexinfo;
	List	   *indexclauses;
...
/* renata todo: change this for now the same as indexPath */
/*
 * SmoothHeapPath represents one [or more - not for now]
 * indexscans that generate TID bitmaps
 ...
 * of a SmoothHeapPath that represents scanning the same index using a
 * SmoothIndexScan.  The startup_cost and total_cost figures of an IndexPath
 * always represent the costs to use it as a regular (or index-only)
 * IndexScan.  The costs of a SmoothIndexScan can be computed using the
 * IndexPath's indextotalcost and indexselectivity.
 */



typedef struct SmoothHeapPath
{
	Path		path;
	Path		*shp_bitmapqual;
